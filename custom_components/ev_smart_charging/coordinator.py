...
from custom_components.ev_smart_charging.const import CONF_PCT_PER_HOUR, CONF_READY_HOUR
...
class EVSmartChargingCoordinator:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._charging = None
        self._charging_pct_per_hour = self.entry.data.get(CONF_PCT_PER_HOUR)
        if self._charging_pct_per_hour is None or self._charging_pct_per_hour <= 0.0:
            self._charging_pct_per_hour = 6.0
        self._ready_hour = int(self.entry.data.get(CONF_READY_HOUR)[0:2])
    ...
    def new_hour(self, date_time: datetime = None):  # pylint: disable=unused-argument
        ...
        if self._charging is not None:
            ...
            turn_on_charging = charging_value is not None and charging_value != 0
