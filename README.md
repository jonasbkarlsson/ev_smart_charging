...
`number.ev_smart_charging_immediate_price` | Number | If the `immediate_price` switch is activated, charging will be done immediately if the electricity price is below this level. Valid values min=-10000, step=0.01, max=10000. Can be set by service call `number.set_value`.
...
`switch.ev_smart_charging_immediate_price` | Switch | Activates charging immediately if the electricity price is lower than a configured level.
